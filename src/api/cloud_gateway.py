...
import src.connectors.aws_connetor as aws
	...
	match req["provider_type"]:
		case "openstack":
			try:
				...
				m.reply= "Collecting the servers from %s cloud resulted in the following exception: %s" % (req["provider_name"],err)
				logging.exception(err)
		case "aws": 
	...
	match req["provider_type"]:
		case "openstack":
			try:
				...
				m.reply= "Collecting the security groups from %s cloud resulted in the following exception: %s" % (req["provider_name"],err)
				logging.exception(err)
		case "aws": 
				pass
...
def create_security_group(req):
	...
	match req["provider_type"]:
		case "openstack":
			try:
				security_group = openstack.create_security_group(req["provider_name"],req["sg_name"])
				update_m = update_security_groups({"provider_name": req["provider_name"], "provider_type" : req["provider_type"]})
				logging.info("Updating the security groups after adding a new group. The returned messages %s" % update_m)
			except Exception as err:
				m.reply= "Creating the security group %s resulted in the following exception: %s" % (req["sg_name"],err)
				logging.exception(err)
		case "aws": 
			pass
			# funtion to be implemented
	...
	match req["provider_type"]:
		case "openstack":
			try:
				...
				m.reply= "Adding the ip %s to group %s resulted in the following exception: %s" % (req["sg_id"], req["ip"], err)
				logging.exception(err)
		case "aws":
			try:
	...
	if "ip" not in req.keys(): 
		...
		if not len(server_query): 
			...
			try: 
				...
				m.reply= "adding the server with ip \"%s\" to the cluster resulted in the following exception: %s" % (req["ip"],err)
				logging.exception(err)
	if m.reply == "": 
		m.status = "SUCCESS!"	
	return m.cast_dict()
